...
import { getDefaultCredentialStore, loadConfig, saveConfig } from "@hachimi/config";
...
export async function startTelegramGateway(options: { runtime?: HarnessRuntime } = {}) {
  ...
  // Token resolution: env var → credential store (bot_token:telegram) → legacy config field.
  const credStore = getDefaultCredentialStore();
  let token =
    process.env.TELEGRAM_BOT_TOKEN ||
    credStore.getSecret("telegram", "bot_token") ||
  ...
  if (!token && process.stdin.isTTY) {
    ...
    try {
      ...
      if (inputToken.trim()) {
        ...
        // Token goes to the credential store (never config.json); whitelist stays in config.
        credStore.setSecret("telegram", "bot_token", token);
        cfg.channels = {
          ...cfg.channels,
          telegram: {
            botToken: "",
        ...
        saveConfig(cfg);
        console.log(`\n✅ 已自动保存 Telegram 配置（Token 已写入本机凭据库）！后续可直接运行。`);
      }
    } finally {
      rl.close();
    }
  ...
  if (!token) {
    console.log(`
❌ 无法启动 Telegram Gateway：未找到 Bot Token。

💡 使用说明：
在设置页「凭据」中添加 telegram Bot Token，或通过环境变量 TELEGRAM_BOT_TOKEN="xxx" 启动。
`);
    return;
  }
