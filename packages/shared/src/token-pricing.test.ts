...
describe("Model-Agnostic Token Usage & Pricing Suite", () => {
  ...
  it("P1.2 clamps misreported cache_read tokens to a sane ceiling", () => {
    // MiniMax 类实现：cache_read 虚报超过非输出 token 总量 → 截断到上限
    const inflated = normalizeUsage({
      input_tokens: 1000,
      output_tokens: 200,
      cache_read_input_tokens: 90000,
      total_tokens: 1200,
    });
    expect(inflated.cacheReadTokens).toBe(1000); // 1200 - 200
    expect(inflated.inputTokens).toBe(1000);
    expect(inflated.outputTokens).toBe(200);

    // 合法数据（cacheRead <= 上限）不误伤
    const legit = normalizeUsage({
      input_tokens: 5000,
      output_tokens: 1500,
      cache_read_input_tokens: 2000,
      total_tokens: 8500,
    });
    expect(legit.cacheReadTokens).toBe(2000);
  });
