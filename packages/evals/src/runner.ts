...
export interface EvalRunnerOptions {
  ...
  parallel?: boolean;
  /**
   * P2: 自动审批所有需确认工具（默认 true）。
   * 评估关注的是工具链是否真实执行，而不是审批 UX；delegate_subagent
   * 升级为 needs_confirm 后，无审批 handler 会被策略直接拒绝。
   */
  autoApprove?: boolean;
}
  ...
  async runCase(evalCase: EvalCase): Promise<EvalResult> {
    ...
    const output = await this.runtime.execute({
      ...
      options: {
        ...
        onToolApproval: this.autoApprove ? async () => true : undefined,
