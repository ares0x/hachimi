...
describe("W1 WorkManager", () => {
  it("list() filters out internal sub-agent works (sub_sess_*)", () => {
    wm.create({ intent: "user work", sessionId: "sess_user_001", kind: "primary" });
    wm.create({ intent: "sub work", sessionId: "sub_sess_internal_001", kind: "primary" });

    const works = wm.list({ kind: "primary" });
    expect(works.some((w) => w.id === "sub_sess_internal_001")).toBe(false);
    expect(works.some((w) => w.id === "sess_user_001")).toBe(true);
  });
  ...
  it("W1.4 projects sub-agent notifications as system rows, not assistant bubbles (P1)", async () => {
    const sid = "sess_proj";
    wm.create({ intent: "调研", sessionId: sid, kind: "primary" });
    await store.append({
      id: generateId("evt_"),
      sessionId: sid,
      type: "assistant_message",
      timestamp: new Date().toISOString(),
      payload: { content: "普通回复" },
    });
    await store.append({
      id: generateId("evt_"),
      sessionId: sid,
    ...
    const { activities } = await wm.listActivities(sid);
    const asst = activities.find((a) => a.content === "普通回复");
    const notif = activities.find((a) => a.type === "system");

    expect(asst?.type).toBe("message");
    expect(asst?.role).toBe("assistant");
    expect(notif).toBeDefined();
    expect(notif?.role).toBe("system");
    expect(notif?.content).toContain("task_sub_1");
    // 通知不再以助手气泡出现
    expect(activities.filter((a) => a.type === "message" && a.role === "assistant")).toHaveLength(1);
  });
});
