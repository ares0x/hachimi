...
export class AgentRunStore {
  ...
  /** Record execution checkpoint for session resumption (t3code/Claude Code pattern). */
  saveCheckpoint(runId: string, checkpoint: AgentRun["checkpoint"]): void {
    const existing = this.getRun(runId);
    if (!existing) return;
  ...
  /**
   * Delete all run records matching a sessionId or workId.
   */
  deleteSessionRuns(sessionId: string): number {
    ...
    try {
      ...
      for (const file of files) {
        ...
        try {
          const run = JSON.parse(readFileSync(filePath, "utf-8")) as AgentRun;
          if (run.sessionId === sessionId || run.workId === sessionId) {
            unlinkSync(filePath);
            deletedCount++;
          }
        } catch {
