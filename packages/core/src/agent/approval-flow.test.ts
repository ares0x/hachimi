...
describe("Agent per-call 审批 handler 优先级", () => {
  it("使用 per-call onToolApproval 放行 needs_confirm 工具", async () => {
    ...
    expect(perCallApproval).toHaveBeenCalledWith("confirm_tool", {}, "needs_confirm", undefined, {
      channel: "api",
...
describe("Agent 拒绝熔断器", () => {
  it("同一工具被拒 2 次后短路，第 3 次不再调用审批 handler", async () => {
    ...
    // 最终返回停止消息（i18n 后支持中/英两种文案）
    expect(reply).toMatch(/已停止|达到最大工具调用轮次|多次工具被拒|Maximum tool call rounds/);
  });
});
