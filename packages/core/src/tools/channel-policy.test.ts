...
describe("PermissionPolicy minimal trust write tightening (P1)", () => {
  const policy = new PermissionPolicy();

  it("minimal trust: safe write-kind tools require approval", () => {
    // write_file 声明为 safe + kind=write → minimal 面仍需审批
    expect(policy.decide("telegram", "write_file", "safe", "minimal", "write")).toBe(
      "require_approval"
    );
    expect(policy.decide("api", "write_file", "safe", "minimal", "write")).toBe("require_approval");
    // 删除类同样收紧
    expect(policy.decide("api", "delete_file", "safe", "minimal", "delete")).toBe(
      "require_approval"
    );
  });

  it("minimal trust: safe read-kind tools remain allowed", () => {
    expect(policy.decide("telegram", "read_file", "safe", "minimal", "read")).toBe("allow");
    expect(policy.decide("api", "grep_search", "safe", "minimal", "read")).toBe("allow");
  });

  it("standard / elevated / full trust still auto-allow safe writes", () => {
    expect(policy.decide("desktop", "write_file", "safe", "standard", "write")).toBe("allow");
    expect(policy.decide("desktop", "write_file", "safe", "elevated", "write")).toBe("allow");
    expect(policy.decide("tui", "write_file", "safe", "full", "write")).toBe("allow");
  });
});
...
describe("Agent + channel — 端到端裁决差异", () => {
  ...
  it("channel=telegram: write_file 策略 require_approval，审批通过后执行", async () => {
    ...
    expect(approval).toHaveBeenCalledWith(
      ...
      "needs_confirm",
      undefined,
      { channel: "telegram", trustLevel: undefined, toolKind: undefined }
    );
    expect(mark.ran).toBe(true);
  });
