...
import type { ActivityPolicy } from "../tasks/activity-policy.js";
...
export class ProactiveScheduler {
  ...
  /** P2.8: 电源/活动感知策略（电池/锁定/活跃租约期跳过触发） */
  private policy?: ActivityPolicy;

  constructor(dataDir = "data", policy?: ActivityPolicy) {
  ...
  async checkAndFire(onTrigger: (task: TriggerTask) => Promise<void>) {
    // P2.8: 用户活跃 / 电池 / 锁定 → 本轮跳过全部触发（不打扰、省电）
    if (this.policy && !this.policy.shouldRunBackground()) {
      return;
    }
    const now = Date.now();
