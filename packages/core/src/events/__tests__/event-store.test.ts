...
describe("FileEventStore append performance (seq cache)", () => {
  it("assigns monotonic seq without re-reading the file on every append", async () => {
    const sid = "sess_seq_cache";
    // Spy on the private reader to prove the hot path never re-scans the file.
    const readSpy = vi.spyOn(store as any, "readAll");

    for (let i = 1; i <= 300; i++) {
      await store.append(makeUserEvent(sid, `msg ${i}`));
    }

    // Only the first append (cache miss) touches the file; the rest use the cache.
    expect(readSpy).toHaveBeenCalledTimes(1);
    readSpy.mockRestore();

    const events = await store.list(sid, { limit: 1000 });
    expect(events.total).toBe(300);
    const seqs = events.events.map((e) => e.seq ?? 0);
    expect(seqs).toEqual([...seqs].sort((a, b) => a - b));
    expect(seqs[0]).toBe(1);
    expect(seqs[seqs.length - 1]).toBe(300);
  });

  it("recomputes seq from the file after a process restart (new store instance)", async () => {
    const sid = "sess_seq_restart";
    for (let i = 1; i <= 5; i++) {
      await store.append(makeUserEvent(sid, `old ${i}`));
    }

    // Simulate a fresh process: new store over the same data dir.
    const freshStore = new FileEventStore(tmpDir);
    await freshStore.append(makeUserEvent(sid, "new"));

    const events = await freshStore.list(sid, { limit: 100 });
    expect(events.total).toBe(6);
    expect(events.events[events.events.length - 1]?.seq).toBe(6);
  });

  it("resets the seq counter after delete", async () => {
    const sid = "sess_seq_delete";
    for (let i = 1; i <= 3; i++) {
      await store.append(makeUserEvent(sid, `x ${i}`));
    }
    await store.delete(sid);

    await store.append(makeUserEvent(sid, "fresh"));
    const events = await store.list(sid, { limit: 10 });
