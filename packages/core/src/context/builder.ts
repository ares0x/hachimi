...
import { wrapToolResultIfUntrusted } from "../security/untrusted-content.js";
import type { SkillRegistry } from "../skills/registry.js";
import type { ToolRegistry } from "../tools/registry.js";
import type { MemoryEntry, Message } from "../types/index.js";
import type { PlanStep } from "../types/work.js";
...
export interface BuiltContext {
  ...
  parts: {
    ...
    /** P1.2: 静态前缀指纹（identity+skills+tools），跨轮变化表示 Prompt 缓存失效 */
...
/** P2.4: 激活技能指令占上下文预算的比例上限（超限降级为摘要 + 按需精读） */
export const SKILL_BUDGET_PERCENT = 0.1;

/** P1.1: 压缩时刻的执行状态快照（计划 / 技能 / 工具组），避免压缩后状态丢失 */
export interface CompactionStateHints {
  /** 未完成的计划步骤（pending / running） */
  planSteps?: PlanStep[];
  /** 当前激活的技能名 */
  activeSkill?: string;
  /** 已加载的工具组 */
...
/** P0.3: 连续压缩失败上限 —— 达到后该 work/session 暂停自动压缩（Claude Code 熔断模式） */
...
export class ContextBuilder {
  ...
  /** P1.2: 上一轮构建的静态前缀指纹（检测技能/工具集变化导致的缓存失效） */
  private lastFingerprint?: string;
  /** P0.3: workId/session → 连续压缩失败次数 */
  private compactionFailures: Map<string, number> = new Map();
  /** P0.3: 已熔断的 work/session（不再自动压缩，直到手动 reset） */
  private compactionBreakers: Set<string> = new Set();

  constructor(identity = MASTER_AGENT_SYSTEM_PROMPT) {
    this.identity = identity;
  }

  /**
   * P1: Compute stable Prompt Cache Hash Key for static invariants
   */
  public computeStaticCacheKey(cacheHint?: {
    identityOverride?: string;
    skills?: SkillRegistry;
    tools?: ToolRegistry;
  }): string {
    const parts: string[] = [];
    parts.push(cacheHint?.identityOverride ?? this.identity);
  ...
  async build(input: ContextBuildInput = {}): Promise<BuiltContext> {
    const dynamicMaxTokens = input.modelId
      ? getModelContextLimit(input.modelId)
      : (input.options?.maxTokens ?? DEFAULT_TOKEN_BUDGET);
    const opts = {
      ...DEFAULT_OPTIONS,
    ...
    if (input.personalContext?.knowledgeRoot) {
      staticBlocks.push(
        `【个人知识库】\n- 知识库根路径: \`${input.personalContext.knowledgeRoot}\`\n- 写入/草稿收件箱: \`${input.personalContext.knowledgeWriteRoot || `${input.personalContext.knowledgeRoot}/_inbox`}\`\n- 极重要指引: 当用户提到“整理我的知识库 / 查看笔记”时，你已挂载了上述知识库路径！请直接通过工具读取或整理该路径下的笔记内容，切勿扫描或混淆为当前项目的代码工作区。`
      );
    }

    // Slim skill list — sorted deterministically to maximize Prompt Cache hits
    let skillsBlock = "【可用技能】\n（无）";
    if (input.skills) {
      const skillList = [...input.skills.list()].sort((a, b) => a.name.localeCompare(b.name));
    ...
    // P3: Slim tool list grouped by semantic kind (Grok Build pattern), sorted deterministically.
    let toolsBlock = "【可用工具】\n（无）";
    if (input.tools) {
      const list = [...input.tools.list()].sort((a, b) => a.name.localeCompare(b.name));
      if (list.length > 0) {
    ...
    // P1.2: 静态前缀指纹 — 检测跨轮技能/工具集变化导致的 Prompt 缓存失效
    const staticFingerprint = this.computeStaticCacheKey({
      identityOverride: input.identityOverride,
      skills: input.skills,
      tools: input.tools,
    });
    const prevFingerprint = this.lastFingerprint;
    const cacheInvalidated = prevFingerprint !== undefined && prevFingerprint !== staticFingerprint;
    this.lastFingerprint = staticFingerprint;
    if (cacheInvalidated) {
      console.warn(
        `[ContextBuilder] Static prefix fingerprint changed (${prevFingerprint} → ${staticFingerprint}) — prompt cache invalidated`
      );
    }
    ...
    if (cacheInvalidated) {
      dynamicBlocks.push(
        "<system-reminder>【上下文缓存已失效】技能/工具集在上一轮后发生变化，静态前缀已重建。请重新评估当前可用的技能与工具，勿依赖旧轮次的缓存状态。</system-reminder>"
      );
    }
    ...
    if (input.activeSkill && input.skills) {
      ...
      if (full) {
        let activeText = `【激活技能：${input.activeSkill}】\n${full.instructions}\n\n请严格按照以上指令完成任务。`;
        // P2.4: 技能上下文预算 — 指令超过预算比例时降级为摘要，避免技能撑爆上下文
        const budgetTokens = Math.max(200, Math.floor(opts.maxTokens * SKILL_BUDGET_PERCENT));
        if (input.tokenEstimator && input.tokenEstimator(activeText) > budgetTokens) {
          const budgetChars = Math.max(400, Math.floor(budgetTokens * 3)); // 中文约 3 chars/token
          const preview =
            full.instructions.length > budgetChars
              ? `${full.instructions.slice(0, budgetChars)}…`
              : full.instructions;
          activeText =
            `【激活技能：${input.activeSkill}】（指令过长，已按上下文预算降级为摘要，约 ${budgetTokens} tokens）\n` +
            `${preview}\n` +
            `如需完整指令请调用 activate_skill 重新获取。`;
        }
    ...
    // 实时数据/行情类查询：优先结构化数据源，限制网页抓取轮数
    dynamicBlocks.push(
      "【实时行情与数据】当用户询问股票/ETF 行情时，优先使用内置 stock_quote 工具" +
        "（symbol 支持 6963.HK、600519.SS、000001.SZ、AAPL 等格式）。" +
        "其他实时数据（汇率、天气、商品价格等）优先调用结构化 JSON API" +
        "（如 api.gold-api.com、api.exchangerate.host 等），不要逐站抓取 HTML。" +
        "网页抓取最多尝试 2-3 轮；仍拿不到就基于已有数据作答，并如实说明数据来源与时效限制。"
    );

    // Tool selection: prefer built-in tools over run_command circumvention
    dynamicBlocks.push(
      "【工具选择】访问文件/目录（包括个人知识库等配置路径）时，" +
        "直接使用 read_file / list_dir / grep_search——它们已配置知识库白名单。" +
        "不要用 run_command 加 shell 命令来读文件或列目录（那会触发额外审批且更慢）。" +
        "工具调用被拒绝时，改用 read_file 等内置工具重试，不要换 run_command 绕路。"
    );

    // H4.2: RAG 动态语义记忆检索装配
    const effectiveMemories = input.memories ? [...input.memories] : [];
    ...
    let hasCompactedMsg = false;
    if (input.history && input.history.length > 0) {
      hasCompactedMsg = input.history.some(
        (m) =>
          m.role === "system" &&
          typeof m.content === "string" &&
          m.content.includes("早期对话历史已归档")
      );
    ...
    // P2: Post-Compact State Re-injection (Claude Code pattern)
    // Re-inject Work Plan, active Skill, and recent file paths when history is compacted
    if (hasCompactedMsg) {
      const stateReinjection = this.buildPostCompactStateReinjection(input);
      if (stateReinjection) {
        dynamicBlocks.push(stateReinjection);
      }
    }
    ...
    if (input.tokenEstimator) {
      ...
      const compactionKey = input.workId ?? "default";
      if (ratio > 95 && input.history && input.history.length > 10) {
        if (this.compactionBreakers.has(compactionKey)) {
          console.warn(
            `[ContextBuilder] Compaction circuit breaker open for '${compactionKey}' — auto-compaction paused`
          );
        } else {
          try {
            // Save reference to the OLD history block before replacing it
            const oldHistoryBlock = historySummary;
            // P1.1: 收集压缩时刻的执行状态（未完成计划 / 激活技能 / 已加载工具组）
            const work = input.workId ? (input.workManager?.get?.(input.workId) as any) : undefined;
            const stateHints: CompactionStateHints = {
              planSteps: work?.plan,
              activeSkill: input.activeSkill,
              toolGroups: input.tools?.getActivatedGroups?.(),
            };
            const compacted = this.compactHistoryBlock(
              input.history,
              opts,
              input.tokenEstimator,
              stateHints
            ...
            // 成功 → 复位熔断计数
            this.compactionFailures.delete(compactionKey);
          } catch (err) {
            // P0.3: 压缩失败计数；连续失败达到阈值 → 熔断（防死锁循环）
            const failures = (this.compactionFailures.get(compactionKey) ?? 0) + 1;
            this.compactionFailures.set(compactionKey, failures);
            if (failures >= MAX_CONSECUTIVE_COMPACTION_FAILURES) {
              this.compactionBreakers.add(compactionKey);
              console.error(
                `[ContextBuilder] Compaction failed ${failures} consecutive times for '${compactionKey}' — circuit breaker opened`
              );
            } else {
              console.error(
                `[ContextBuilder] Compaction failed (${failures}/${MAX_CONSECUTIVE_COMPACTION_FAILURES}): ${
  ...
  /** P0.3: 查询指定 work/session 的压缩熔断状态 */
  isCompactionBreakerOpen(workId?: string): boolean {
    return this.compactionBreakers.has(workId ?? "default");
  }

  /** P0.3: 手动压缩成功后复位熔断（恢复自动压缩） */
  resetCompactionBreaker(workId?: string): void {
    const key = workId ?? "default";
    this.compactionBreakers.delete(key);
    this.compactionFailures.delete(key);
  }
  ...
  private compactHistoryBlock(
    ...
    stateHints?: CompactionStateHints
  ): string {
    ...
    // P1.1: 执行状态快照 — 压缩后仍保留「正在做什么」的可见状态
    const pendingPlan = (stateHints?.planSteps ?? []).filter(
    ...
    if (pendingPlan.length > 0) {
      stateLines.push(
        "- 当前计划（未完成）:",
        ...pendingPlan.map(
          (s) => `  - [${s.status}] ${s.title}${s.description ? ` — ${s.description}` : ""}`
        )
      );
    }
    if (stateHints?.activeSkill) {
      stateLines.push(`- 已激活技能: ${stateHints.activeSkill}`);
    }
    if (stateHints?.toolGroups && stateHints.toolGroups.length > 0) {
      stateLines.push(`- 已加载工具组: ${stateHints.toolGroups.join(", ")}`);
    }
    const stateSection =
      stateLines.length > 0 ? `\n\n【执行状态快照（压缩时刻）】\n${stateLines.join("\n")}` : "";
  ...
  private sanitizeMessageContent(m: Message, maxBytes: number): Message {
    ...
    // P2-B2: tag untrusted tool output (web / MCP) before it reaches the model.
    if (m.role === "tool") {
      contentStr = wrapToolResultIfUntrusted(m.name, contentStr);
    }
  ...
  /**
   * P2: Post-Compact State Re-injection (Claude Code pattern).
   * Generates a focused snapshot of Work Plan, active skill, and recently accessed files
   * so the Agent retains immediate operational state after history compaction.
   */
  private buildPostCompactStateReinjection(input: ContextBuildInput): string | null {
    const lines: string[] = ["【状态重注入 (Post-Compact State Re-injection)】"];
    let count = 0;

    // 1. Active Work Plan
    if (input.workId && input.workManager) {
      try {
        const work = input.workManager.get(input.workId);
        if (work && work.plan && work.plan.length > 0) {
          lines.push(`- 当前任务目标: ${work.title || work.goal || "(无)"}`);
          lines.push("- 当前 Work Plan 步骤:");
    ...
    // 2. Recent Workspace Files accessed in history
    if (input.history && input.history.length > 0) {
      ...
      for (let i = input.history.length - 1; i >= 0; i--) {
        ...
        if (msg.tool_calls) {
          for (const tc of msg.tool_calls) {
            const pathArg = (tc.arguments?.path ??
              tc.arguments?.filepath ??
              tc.arguments?.targetfile) as string;
            if (pathArg && typeof pathArg === "string" && !accessedFiles.includes(pathArg)) {
              accessedFiles.push(pathArg);
              if (accessedFiles.length >= 5) break;
            }
          }
        }
        if (accessedFiles.length >= 5) break;
      ...
      if (accessedFiles.length > 0) {
        lines.push(`- 最近调用的关键文件: ${accessedFiles.map((f) => `\`${f}\``).join(", ")}`);
        count++;
      }
    }
