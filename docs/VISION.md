...
## 4. The final product — end-state shape & differentiation

### End-state shape: a personal runtime you own, plus one standing console

Hachimi's end state is not "an app that talks to models" — it is the user's own
**agent brain** with a standing control surface on every device they use:

- **One brain, resident on the machine.** A daemon hosts the runtime, survives
  UI restarts, and is reachable from Desktop, Web, TUI, CLI, Telegram, and the
  HTTP API. Closing a window never kills the work; killing the process never
  loses the line (append-only events + recovery).
- **Work as the only unit of record.** A chat, a task, and a folder-bound
  project are the same object at different weights. Everything the user has
  asked for lives in resumable, auditable lines — never scattered across
  surface-specific silos.
- **Memory as a first-class, user-visible surface.** A memory library UI,
  per-Work incognito, audit log, and editable permission rules make "what the
  brain remembers and is allowed to do" inspectable and controllable — not a
  hidden black box.
- **Self-evolution behind a human gate.** The runtime distills trajectories
  into skill proposals; the user reviews and accepts; skills then become normal
  capabilities. The product grows with its owner, by consent.
- **Portable by design.** Memory bundle export/import means the brain can move
  machines without vendor lock-in or data hostage.

### Differentiation — the axes that matter

| Adjacent product | What it does well | Hachimi's differentiator |
| --- | --- | --- |
| ChatGPT / Claude web apps | Instant, polished chat | Hachimi is **local-first and owned**: swappable models, portable data, auditable events, editable policy — the assistant is a tool you run, not a website you visit. |
| Claude Code / Codex CLI | Deep coding-in-repo sessions | Hachimi is **weight-agnostic**: a folder-bound coding project is one kind of Work, not the whole product. Casual questions, tasks, and projects share one resumable line. |
| Maka / local Electron AI workbenches | Desktop-first agent UX | Hachimi's center is the **runtime layer** (Work / events / policy); Desktop is one projection of it — TUI, Web, Telegram, and API stay first-class with one brain and no split state. |
| Agent frameworks (LangChain, etc.) | Flexible building blocks | Hachimi is a **productized runtime**: batteries-included UI, memory, approvals, audit, portable bundles — usable without assembling an agent stack yourself. |
| "AI notebooks" / chat logs | Familiar conversation record | Hachimi's record is an **execution line with process**: goal / plan / activity / artifacts, not just messages — it shows "what it did and why" as clearly as "what it said". |

### What the user feels at the end state

- Owns the runtime on this machine — quitting the app never loses work.
- Switches models or surfaces mid-line without losing the thread.
- Lets the agent touch files and run commands because policy and the ledger
  make every action inspectable.
- Watches the agent get better over time through skills it proposed and the
  user accepted — improvement is earned, not assumed.

---

## 5. What “done” looks like (user-perceived end state)
...
## 6. Current state vs end state — verified, not self-reported

Ratings reflect **code/wiring checks**, not phase checkboxes alone.
...
| Work model + API + CLI | **Solid** | First-class internally; `workspaceRoot` landed (`RuntimeInput → Work → tool exec`); `uiKind` still pending |
...
| Trust & memory UX | **Shipped** | Memory library UI, incognito per-Work, audit log, permission-rules editor, offline recovery (P1 productization batch) |
| Builtin tools | **Solid** | Streaming FS, grep, replace, shell, memory |
| Sandbox | **Solid** | PathJail/ToolSandbox follow request-scoped `workspaceRoot` from Work; tool exec env scrubbed of secrets (`isSensitiveEnvKey`); concurrent Works per-folder supported at runtime level |
...
## 7. Operating discipline: verify the wire, not just the definition
...
## 8. Implication for near-term product work (derived, not a full roadmap)
