...
> `ROADMAP.md`, which kept tracking the original A–G scheme. This file is
> reverted to match that lettering — no prose content is lost, only phase
> letters. One substantive addition survives from the A–J attempt: a
...
> resolved. Newly found structural issues are fixed in place (or tracked as
> issues) instead of being duplicated here.
...
yet built (`wechat`, `slack`). Keep the union aligned with actually shipped
channels; adding a new channel means extending both the union and its policy
surface mapping.
...
model-family (`claude`) names in one flat union — acceptable while the registry
keeps transport selection explicit per connection.
...
the current codebase, were fixed in place before the daemon became a second
consumer of the core: deep relative imports bypassing `@hachimi/core`'s public
API, an incomplete `index.ts` export surface, the `ChannelType` closed-union
coupling, provider-naming conflation, and config's legacy-field sunset path.
