...
```css
--font-sans:
  "Inter", "PingFang SC", "Noto Sans SC", "Source Han Sans SC", "Microsoft YaHei", system-ui,
  sans-serif;
...
_Hachimi Design System — quiet tools, durable local assistant, honest surfaces._
...
| Generation                | Interaction unit                              | UI center of gravity                       |
| ------------------------- | --------------------------------------------- | ------------------------------------------ |
| Chatbot (2023)            | Message                                       | Bubble transcript                          |
| Copilot (2024)            | Suggestion in a host app                      | Side panel                                 |
| **Agent workbench (now)** | **Run** (goal → plan → tool calls → artifact) | **Session stream + inspectable run trace** |

Design consequences, all shipped in the reference implementation:

1. **The run, not the message, is the primitive.** A turn renders as _plan → tool strip → answer_, so a
   long-running or resumed run is legible without opening logs.
2. **Plan before act.** Every non-trivial run shows a 2–5 step plan block with per-step status. The plan is
   the contract the user can interrupt.
3. **Tools collapsed by default, auditable on demand.** `N tool calls` strip; expanded rows show name,
   status, sandbox badge, args (mono), duration, result summary. Never auto-expand.
4. **Human-in-the-loop is docked, not modal.** Write / send / spend actions raise a permission sheet above
   the composer with _allow once · always in session · deny_. The composer disables while a decision is pending.
5. **Memory is a visible, editable surface.** Recall must be inspectable (kind, age, hit count) and forgettable.
   Hidden memory is a trust bug, not a feature.
6. **Budget is chrome, not a settings page.** Tokens, cost, and trust level live in the Inspector footer/budget card.
7. **Intent bar over menu trees.** ⌘K resolves goals, modes, and actions; the sidebar stays a _quiet_ index.
8. **One brain, many scenes.** Modes (chat / code / research / write) are tints and tools on one conversation
   surface — never a roster of peer agent avatars (§1.3).
9. **Local-first honesty.** Sandbox, PathJail, and denial states are shown in plain language with a next step.

### 17.2 Canonical layout

```text
┌──────────────┬──────────────────────────────┬──────────────────┐
│ Sidebar 264  │ Session (min-w-0 flex-1)     │ Inspector 320    │
│ mark+wordmark│ header 56: title · model ·   │ tabs: Activity / │
│ status card  │   run dot · Inspector toggle │  Sources /Memory │
│ New session  │ stream: max-w 52rem          │ run trace        │
│ Modes ⌘1-4   │   user bubble / assistant    │ budget card      │
│ Sessions     │   document flow              │ policy footer    │
│ ⌘K · Memory  │ [permission dock]            │                  │
│ Bundle · Set │ composer + mono hints        │                  │
└──────────────┴──────────────────────────────┴──────────────────┘
```

- Header height is **56px** (was ~44px in §9.4): it carries title + run status + model meta.
- Sidebar is persistent ≥1024px, drawer below. Inspector is persistent ≥1280px, overlay 768–1279px, sheet below.
- Desktop (Electron/Tauri) reuses this exact grid; only window chrome (§11) differs.

### 17.3 Send key

Baseline updated: **Enter sends, ⇧Enter newlines** in the agent composer (matches the shipped surfaces and
user expectation); ⌘Enter is reserved for "run the highlighted intent" from the command palette.

### 17.4 Conformance status

The Web reference workbench in this repository is **conformant**: flat token surfaces, light default,
borders over blur, no radial gradients, no glassmorphism, ink-teal primary, opacity-pulse status dot.
§13.4's non-conformance note applies only to the legacy `packages/channels/web` stylesheet.

### 17.5 Changelog 1.2.0

- Added §17: agent form analysis, canonical triptych with sizes, run-as-primitive rules.
- Header height 56px; Enter-to-send baseline.
- New tokens surfaced as utilities: `pulse-status`, `nav-rail`, `wordmark`, `scroll-quiet`, `enter-rise`.
- Renamed the secondary cool accent token to `--brand-accent` so it cannot collide with shadcn's `--accent` hover surface.
